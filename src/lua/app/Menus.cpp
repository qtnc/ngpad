...
static bool MenuBarIsEnabled (wxMenuBar& mb, int id) {
return mb.IsEnabled(id);
}
...
.method("check", SYNC(&MenuItemCheck))
//M Enable or disable this item
//P enabled: boolean: true: the enabled or disabled state to apply
.method("enable", SYNC(&MenuItemEnable))
//A Menu: the submenu included in this item
.property("subMenu", SYNC(&wxMenuItem::GetSubMenu), SYNC(&wxMenuItem::SetSubMenu))
//A Menu: the parent menu of this item
.property("menu", SYNC(&wxMenuItem::GetMenu), SYNC(&wxMenuItem::SetMenu))
...
.property("helpString", SYNC(&wxMenuItem::GetHelp), SYNC(&wxMenuItem::SetHelp))
//A string: the text label of this item
.property("label", SYNC(&wxMenuItem::GetItemLabel), SYNC(&wxMenuItem::SetItemLabel))
//G string: the text label of this item, without including accelerators
.getter("labelText", SYNC(&wxMenuItem::GetItemLabelText))
//A boolean: Tells if this item is currently checked
.boolProperty("checked", SYNC(&wxMenuItem::IsChecked), SYNC(&MenuItemCheck))
//G boolean: Tells if this item is checkable, i.e. if it has a checkbox
.getter("checkable", SYNC(&wxMenuItem::IsCheckable))
...
.boolProperty("enabled", SYNC(&wxMenuItem::IsEnabled), SYNC(&MenuItemEnable))
//G boolean: Tells if this item is a separator
.getter("separator", SYNC(&wxMenuItem::IsSeparator))
//G integer: unique identifier of this item
.getter("id", SYNC(&wxMenuItem::GetId))
...
.constructor(SYNC(&MenuConstructor))
.referenceEquals()
...
.method("add", SYNC(&MenuAdd), { "label", "subMenu", "helpString", "position", "id", "checkable", "radio", "checked" })
...
.method("addSeparator", SYNC(&MenuAddSeparator))
...
.method("isChecked", SYNC(&wxMenu::IsChecked))
...
.method("show", SYNC(&MenuShowPopup))
...
.method("chooseOne", SYNC(&showPopupMenu))
.pop();
...
.method("add", SYNC(&MenuBarAdd), {"label", "menu", "position"})
//M Remove a menu from this menu bar
//P index: integer: nil: 1-based index of the menu to remove
.method("remove", SYNC(&wxMenuBar::Remove))
.method("replace", SYNC(&wxMenuBar::Replace))
...
.method("enableTop", SYNC(&wxMenuBar::EnableTop))
.method("isEnabledTop", SYNC(&wxMenuBar::IsEnabledTop))
.property("helpString", SYNC(&wxMenuBar::GetHelpString), SYNC(&wxMenuBar::SetHelpString))
...
.method("getMenu", SYNC(&MenuBarGetMenu))
//M Return the number of menus present in this menu bar
//R integer: the number of menus in this menu bar
.method("getMenuCount", SYNC(&wxMenuBar::GetMenuCount))
//M Return the number of menus present in this menu bar
//R integer: the number of menus in this menu bar
.method("__len", SYNC(&wxMenuBar::GetMenuCount))
...
.method("__altindex", SYNC(&MenuBarGetMenu))
.property("menuLabel", SYNC(&wxMenuBar::GetMenuLabel), SYNC(&wxMenuBar::SetMenuLabel))
.method("getMenuLabelText", SYNC(&wxMenuBar::GetMenuLabelText))
.pop();
